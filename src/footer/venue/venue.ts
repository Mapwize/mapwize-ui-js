...
export class FooterVenue extends DefaultControl {
  ...
  public refreshLocale () {
    if (!$(this.map._container).hasClass(uiConfig.SMALL_SCREEN_CLASS)) {
      ...
      this._container.find('#mwz-universe-button, #mwz-language-button').tooltip({ container: this.map._container, sanitize: false, sanitizeFn: content => content, popperConfig: {} })
    }
  }
  // ---------------------------------------
  // Privates methods
