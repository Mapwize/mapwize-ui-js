...
MapwizeUI version `3.0.7` uses Mapwize SDK version `4.4.3`.
...
```html
<script type="text/javascript" src="https://cdn.jsdelivr.net/npm/mapwize-ui@3.0.5"></script>
```
...
Mapwize UI has a default interface to display place details when a place is selected. This interface can be modified using the `onDetailsWillBeDisplayed?: (detailsViewConfig: DetailsViewConfig) => DetailsViewConfig` options.

This methods is called each time the Mapwize UI SDK is going to display the Place Details View. The `detailsViewConfig` argument is the configuration computed by the SDK. The returned object is the configuration that will be used by the SDK to display the information. By default, the SDK display the configuration given in the argument but you can decide to return a custom DetailsViewConfig to change some element in the interface.

### DetailsViewConfig

DetailsViewConfig is the object that you get from the `onDetailsWillBeDisplayed` method
...
type DetailsViewConfig = {
	...
	smallViewButtons: DetailsViewButton[] // The list of buttons displayed in the unexpanded view
}
```
...
export type DetailsViewButton = {
	type: DetailsViewButtonType // The type of the button. Mainly use to decide if you want to keep it or remove it
...
export type DetailsViewRow = {
	...
	informationAvailable: boolean // True if the information is available. If false, the row displayed a placeholder (eg: 'Phone numer is not available')
...
Those types are defined to help the developer to detect which information is contained in an element. All component in the Configuration generated by the SDK contains a type that is not the `custom` type. The type `custom` should be use by the developer that want to add a custom view.
...
In order to make customization a bit easier, four methods are available to build components that follow the MapwizeUI design by default.
...
You are not limited to those methods. You can build your own HTMLElement and display them but you have to handle the view constraints yourself.
