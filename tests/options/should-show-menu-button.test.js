...
mwzDescribe(testSuites, function () {
  mwzTest('show menu button', function (callbackTest) {
    MapwizeUI.map({
      ...
      if ($('#mapwize .mwz-menu-button').length !== 1) {
        callbackTest('#mapwize .mwz-menu-button should exist when onMenuButtonClick function is set')
      } else {
        callbackTest(null)
      }
  ...
  mwzTest('menu button hidden by default', function (callbackTest) {
    MapwizeUI.map({
      ...
      if ($('#mapwize .mwz-menu-button').length !== 0) {
        callbackTest('#mapwize .mwz-menu-button should not exist when onMenuButtonClick function is not set')
      } else {
        callbackTest(null)
      }
    }).catch(function (e) { callbackTest(e.toString()) })
